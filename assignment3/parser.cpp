...
static bool have_constructor = false;
...
ast parse_field_var_dec()
{
    ...
    vector<ast> decs;
    //following the grammar rules given, adding things in to table
    mustbe(tk_field);
    string type = token_spelling(mustbe(tg_starts_type));
    Token name = mustbe(tk_identifier);
...
Token parse_type()
{
    ...
    Token type = nullptr;
    //different cases according to rules given
    if (have(tk_int))
    {
        type = mustbe(tk_int);
    }
...
ast parse_subr_decs()
{
    ...
    vector<ast> subrs;
    //these three cases will be handled here
...
ast parse_function()
{
    ...
    //using the rules to make the input
    mustbe(tk_function);
...
ast parse_param_list()
{
    ...
    if (have(tg_starts_type))
    {
        ...
        Token name = mustbe(tk_identifier);
        param_list.push_back(declare_variable(name, type, "argument"));
        while (have(tk_comma))
        {
            mustbe(tk_comma);
            string type = token_spelling(parse_type());
    ...
    pop_error_context();
    return create_param_list(param_list);
}
...
ast parse_statements()
{
    ...
    //if there's anything that can start statement, run parse
...
//
//let can only change, not declare so lookup only
ast parse_let()
{
    push_error_context("parse_let()");

    mustbe(tk_let);
    ast var = lookup_variable(mustbe(tk_identifier));
    //if have something to start it, parse
    if (have(tk_lsb))
    {
...
ast parse_do()
{
    ...
    if (have(tk_stop))
    {
        ...
        jack_var var = jsymbols_lookup(symbol_tables, token_spelling(name));

        //if exist, it's a method, if not, it's a function
        if (var != nullptr)
        {
            ast var = lookup_variable(name);
            call = create_call_as_method(get_var_type(var), var, subr_call);
        }
        else
        {
...
ast parse_expr()
{
    ...
    vector<ast> expr;
    //must be alternate so one before loop
...
ast parse_var_term()
{
    ...
    //if there is a full stop, find it in table
    else if (have(tk_stop))
    {
        ...
        {
            ast var = lookup_variable(name);
            return create_call_as_method(get_var_type(var), var, subr_call);
        }
        else
        {
            return create_call_as_function(token_spelling(name), subr_call);
        }
    }
    else if (have(tk_lrb))
    {
    ...
    jack_var var = jsymbols_lookup(symbol_tables, token_spelling(name));
    pop_error_context();
    return create_var(token_spelling(name), jack_var_segment(var), jack_var_offset(var), jack_var_type(var));
}
...
Token parse_unary_op()
{
    ...
    Token unary_op = nullptr;

    if (have(tg_unary_op))
    {
        unary_op = current_token();
    }
